...
async function salvarJogo(e) {
    ...
    if(document.getElementById('jogo-screen1').files[0]) formData.append('screenshot1', document.getElementById('jogo-screen1').files[0]);
    if(document.getElementById('jogo-screen2').files[0]) formData.append('screenshot2', document.getElementById('jogo-screen2').files[0]);
    if(document.getElementById('jogo-screen3').files[0]) formData.append('screenshot3', document.getElementById('jogo-screen3').files[0]);

    try {
        const res = await fetch(`${API_URL}/admin/jogos`, { method: 'POST', headers: getHeaders(true), body: formData });
...
async function deletarJogo(id) {
    ...
    try {
        await fetch(`${API_URL}/admin/jogos/${id}`, { method: 'DELETE', headers: getHeaders() });
        carregarJogos();
    ...
    try {
        ...
        usuarios.forEach(u => {
            tbody.innerHTML += `
                <tr>
                    <td>${u.id}</td>
                    <td>${u.nome} ${u.adm ? '<span style="color:red">(Admin)</span>' : ''}</td>
                    <td>${u.email}</td>
                    <td><input type="number" id="saldo-${u.id}" value="${u.saldo}" style="width: 80px; padding: 5px; background: #1a1a2e; color: white;"></td>
                    <td><input type="number" id="pontos-${u.id}" value="${u.pontos}" style="width: 80px; padding: 5px; background: #1a1a2e; color: white;"></td>
                    <td><button class="btn-action btn-yellow" onclick="atualizarUsuario(${u.id})">Salvar</button></td>
                </tr>
            `;
        });
    } catch(e) { console.error(e); }
...
async function atualizarUsuario(id) {
    ...
    try {
        await fetch(`${API_URL}/admin/usuarios/${id}`, { 
            method: 'PUT', headers: getHeaders(), body: JSON.stringify({ saldo, pontos }) 
        });
...
async function salvarCupom(e) {
    ...
    try {
        await fetch(`${API_URL}/admin/cupons`, { method: 'POST', headers: getHeaders(), body: JSON.stringify(data) });
        alert('Cupom cadastrado!');
        document.getElementById('form-cupom').reset();
        carregarCupons();
    } catch(err) { alert('Erro ao cadastrar cupom.'); }
}

async function deletarCupom(id) {
    if(!confirm('Deletar este cupom?')) return;
    try {
        await fetch(`${API_URL}/admin/cupons/${id}`, { method: 'DELETE', headers: getHeaders() });
        carregarCupons();
    } catch(err) { alert('Erro ao deletar.'); }
}

window.onload = () => { if(verificarPermissao()) carregarJogos(); };